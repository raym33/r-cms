...
function ccms_admin_handle_authenticated_post(string $action, array &$data, array $currentAdmin): void
{
    switch ($action) {
        ...
        case 'save_ai_settings':
            ...
            $data['local_ai'] = ccms_ai_settings_input($_POST, $data['local_ai'] ?? []);
            ccms_push_audit_log($data, 'ai.settings_updated', 'LM Studio settings updated', $currentAdmin, [
                ...
                'preferred_pack_id' => $data['local_ai']['preferred_pack_id'] ?? 'auto',
            ]);
            ccms_save_data($data);
        ...
        case 'probe_ai':
            ccms_require_capability('ai_generate');
            $settings = ccms_ai_settings_input($_POST, $data['local_ai'] ?? []);
        ...
        case 'ai_generate_page':
            ...
            $brief = [
                ...
                'pack_id' => trim((string) ($_POST['pack_id'] ?? ($data['local_ai']['preferred_pack_id'] ?? 'auto'))),
            ];
            ...
            $data['local_ai']['preferred_pack_id'] = (string) ($payload['_meta']['pack_id'] ?? $brief['pack_id'] ?? 'auto');
            if (isset($_POST['apply_site_branding']) && is_array($payload['site'] ?? null)) {
                ...
                $data['site']['theme_preset'] = ccms_normalize_theme_preset((string) ($payload['site']['theme_preset'] ?? $data['site']['theme_preset']));
                $data['site']['font_pairing'] = ccms_normalize_font_pairing((string) ($payload['site']['font_pairing'] ?? $data['site']['font_pairing']));
            ...
            ccms_push_audit_log($data, 'ai.page_generated', 'Page generated from Studio', $currentAdmin, [
                ...
                'pack_id' => $payload['_meta']['pack_id'] ?? $brief['pack_id'] ?? 'auto',
            ]);
            ccms_save_data($data);
